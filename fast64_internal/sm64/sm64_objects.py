...
from .sm64_level_writer import get_level_props_context
...
class PuppycamVolume:
	def __init__(self, permaswap, functionName, position, rotation, scale, emptyScale, camPos, camFocus, room, addflags, removeflags, camYaw, camPitch, camZoom, volumeShape, angleptr, fov: int):
		...
		self.position = position
		self.scale: mathutils.Vector = mathutils.Vector((scale[0], scale[2], scale[1])) * emptyScale
		self.rotation = rotation
		...
		if specialsOnly:
			...
			if obj.sm64_obj_type == 'Object':
				...
				area.puppycamVolumes.append(
					PuppycamVolume(
						puppycamProp.puppycamVolumePermaswap, puppycamProp.puppycamVolumeFunction,
						translation,
						rotation.to_euler(),
						scale,
...
class SM64ObjectPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		if obj.sm64_obj_type == 'Object':
			...
			prop_split(column, puppycamProp, 'fov', 'Camera FOV')
...
class PuppycamSetupCamera(bpy.types.Operator):
	...
	def execute(self, context):
		...
		bpy.data.cameras[cameraObject].angle = math.radians(bpy.context.active_object.fast64.sm64.puppycam.fov * (4/3))
