...
class BaseDisplayListNode:
	...
	def get_c_func_macro(self, base_cmd: str):
		return f'{base_cmd}_WITH_DL' if self.hasDL else base_cmd

	def c_func_macro(self, base_cmd: str, *args: str):
		'''
			Supply base command and all arguments for command.
			if self.hasDL:
				this will add '_WITH_DL' to the command, and
...
class TransformNode:
	...
	def getDrawLayers(self):
		if self.node is not None and self.node.hasDL:
			...
			drawLayers = set()
		for child in self.children:
			if hasattr(child, 'getDrawLayers'):
...
class CustomNode:
	...
	def to_c(self):
		# return f"{self.command}({join_c_args(self.args)}),"
